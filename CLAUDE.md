...
**mikro-dto-generator** is a pnpm monorepo that provides CLI tools for generating NestJS DTOs and resource operations from MikroORM entities. The project uses pnpm workspaces and consists of two packages: `generator` (the core CLI tool) and `test-nest-app` (a test application demonstrating usage).
...
# Test generating resource from entities
pnpm -C packages/test-nest-app generate-resource
...
1. **Entry Points**
   - `packages/generator/src/cli.ts` - Main DTO generator CLI
   - `packages/generator/src/resource-cli.ts` - resource generator CLI
...
1. `mikro-dto-generator.config.ts`
2. `mikro-dto-generator.config.js`
3. `.mikro-dto-generator.config.ts`
4. `.mikro-dto-generator.config.js`
...
1. **Making Changes to Generator Logic**
   ...
   - Test changes for resource generation using `cd packages/test-nest-app && rm -rf src/generated/{resource-path} && pnpm generate-resource -e src/entities/{entity-name}.ts -o src/generated`


## Important Notes

- This is a pnpm workspace - always use `pnpm` instead of `npm`
- Generated files are placed in `src/generated/mdg/` by default
