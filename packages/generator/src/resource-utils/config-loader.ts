...
import { findAndLoadConfig as sharedFindAndLoadConfig } from '../shared-config/config-loader';

/**
 * Find and load DTO generator configuration file using shared config loader
 */
export async function findAndLoadDtoConfig(): Promise<DtoGeneratorConfig | null> {
  const config = await sharedFindAndLoadConfig();
  if (config) {
    console.log(`✓ Loaded DTO configuration`);
  } else {
    console.log('⚠️  No DTO configuration file found');
  }
  return config;
}
 ...
 */
export function getDtoImportPath(entityFileName: string, config: DtoGeneratorConfig, entityFilePath?: string, crudOutputPath?: string): string {
 ...
 */
export function getEntityDtoImports(entityFileName: string, config: DtoGeneratorConfig, entityFilePath?: string, crudOutputPath?: string): {
  dtoImport: string;
