...
export interface CreateDtoGeneratorConfig {
  project: Project;
...
export interface DtoOptions {
  exclude?: boolean | ('create' | 'update' | 'find' | 'findMany')[];
  include?: string[];
}

/**
 * Generate Create DTO files for all entities in the source files
 * @param config Generator configuration
 */
export function generateCreateDtoFiles(config: CreateDtoGeneratorConfig): void {
    ...
    for (const classDeclaration of classes) {
      ...
      if (className) {
        try {
          ...
          console.error(`[ERROR] Failed to generate Create DTO for entity '${className}' in file '${sourceFile.getFilePath()}':`, error);
...
/**
 * Generate a Create DTO file for a single entity
 */
export function generateCreateDtoForEntity(
  ...
  const dtoFileName = `${className.toLowerCase()}.create.dto.ts`;
...
/**
 * Process a property and add it to the Create DTO class
 */
...
/**
 * Check if a property should be excluded based on DtoOptions decorator for Create DTO
 */
function shouldExcludeProperty(property: PropertyDeclaration): boolean {
  return shouldExcludePropertyForOperation(property, 'create');
}

/**
 * Process an entity property and add it to the Create DTO class
 */
function processEntityProperty(
  ...
  if (isNullable && !isOptional) {
    ...
    // For Create DTOs, make nullable fields optional instead of required
    // This changes the behavior from description!: string | null; to description?: string | null;
  }
  ...
  // For Create DTOs, make nullable fields optional instead of required with !
  const isOptionalInCreateDto = isOptional || isNullable;

  const propertyDef = dtoClass.addProperty({
    name: property.getName(),
    type: typeText,
    hasQuestionToken: isOptionalInCreateDto,
    hasExclamationToken: !isOptionalInCreateDto, // Add ! for required fields that are not nullable
  });
...
/**
 * Process a relation property and add it to the Create DTO class
 */
    ...
    if (targetEntity) {
      ...
      try {
        ...
        if (error instanceof EntityResolutionError) {
          ...
          const context = createErrorContext(null, property, 'create-dto-generation', 'createDto');
