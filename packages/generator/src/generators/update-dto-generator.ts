...
export interface UpdateDtoGeneratorConfig {
  project: Project;
...
/**
 * Generate Update DTO files for all entities in the source files
 * @param config Generator configuration
 */
export function generateUpdateDtoFiles(config: UpdateDtoGeneratorConfig): void {
    ...
    for (const classDeclaration of classes) {
      ...
      if (className) {
        try {
          ...
          console.error(`[ERROR] Failed to generate Update DTO for entity '${className}' in file '${sourceFile.getFilePath()}':`, error);
...
/**
 * Generate an Update DTO file for a single entity
 * Primary key is required, all other fields are optional
 */
export function generateUpdateDtoForEntity(
  ...
  const dtoFileName = `${className.toLowerCase()}.update.dto.ts`;
...
/**
 * Process a property and add it to the Update DTO class
 * Primary key is required, all other fields are optional
 */
function processProperty(
  ...
  if (isPrimaryKey) {
    // Primary key is required in Update DTO
    processPrimaryKeyProperty(
      property,
      propertyTypeText,
      isOptional,
      isNullable,
    ...
    // All other entity properties are optional in Update DTO
    ...
    // All relation properties are optional in Update DTO
...
/**
 * Process a primary key property and add it to the Update DTO class (required)
 */
function processPrimaryKeyProperty(
  property: PropertyDeclaration,
  ...
  // For nullable primary keys, modify the type to include | null
...
/**
 * Process an entity property and add it to the Update DTO class (optional)
 */
function processEntityProperty(
  ...
  // For Update DTOs, all non-primary key fields are optional
  apiPropertyArgs.push("required: false");
  ...
  const propertyDef = dtoClass.addProperty({
    name: property.getName(),
    type: typeText,
    hasQuestionToken: true, // All non-primary key fields are optional in Update DTO
  });
  ...
  if (apiPropertyArgs.length > 0) {
    ...
    // For simple optional properties with no special attributes, still add ApiProperty
    // but without arguments to keep it clean
    propertyDef.addDecorator({
      name: "ApiProperty",
      arguments: [""],
...
/**
 * Process a relation property and add it to the Update DTO class (optional)
 */
    ...
    if (targetEntity) {
      ...
      try {
        ...
        if (error instanceof EntityResolutionError) {
          ...
          const context = createErrorContext(null, property, 'update-dto-generation', 'updateDto');
      ...
      if (apiPropertyArgs.length > 0) {
        ...
        // For simple optional relations with no special attributes, still add ApiProperty
        // but without arguments to keep it clean
        propertyDef.addDecorator({
          name: "ApiProperty",
          arguments: [""],
...
/**
 * Check if a property should be excluded based on DtoOptions decorator for Update DTO
 */
function shouldExcludeProperty(property: PropertyDeclaration): boolean {
  return shouldExcludePropertyForOperation(property, 'update');
}